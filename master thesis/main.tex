...
An isothermal damped harmonic oscillator is a port-Hamiltonian system. Its Dirac structure can be defined by a skew-symmetric matrix. The subsystems interact with each other via the interconnection
...
where $e_{R}=p/m$ and $f_{R}=de_{R}$. The pair $(f_R, e_R)$ is an energy-dissipating port of the damping and $d$ is the damping coefficient. Referring to \ref{eq:Hamiltonian_udho}, the system of ODEs can be expressed by
