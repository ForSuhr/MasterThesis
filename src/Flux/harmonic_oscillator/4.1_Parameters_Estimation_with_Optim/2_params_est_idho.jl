...
randomized = VectorOfArray([(sol(t[i]) + .02randn(3)) for i in 1:length(t)]) ## sol(t[i]) means u(t) at the timepoint t=i
...
prob_ODEfunc_idho = ODEProblem(ODEfunc_idho, u0, tspan, est_params)
sol = solve(prob_ODEfunc_idho, Tsit5(), saveat = tsteps)
est_data = Array(sol)
x_axis_est_data = est_data[1,:]
y_axis_est_data = est_data[2,:]
z_axis_est_data = est_data[3,:]
plt = plot!(x_axis_est_data, y_axis_est_data, label="Estimation")
