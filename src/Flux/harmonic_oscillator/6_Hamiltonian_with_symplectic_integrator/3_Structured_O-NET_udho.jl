...
sensealg=InterpolatingAdjoint(autojacvec=ReverseDiffVJP(true))))
plt = plot(q_ode_data, p_ode_data, xlims=(-2,2), ylims=(-2,2), lw=5, label="Ground truth", linestyle=:solid)
plt = plot!(trajectory_estimate_structured_O_NET[1,:], trajectory_estimate_structured_O_NET[2,:], lw=5,  label = "Structured O-NET", linestyle=:dot)

# Calculate training loss
training_error_structured_O_NET = sum((ode_data .- trajectory_estimate_structured_O_NET).^2)/2/200
begin
  min_error = (findmax(sum((ode_data[:, 1:50] .- trajectory_estimate_structured_O_NET[:, 1:50]).^2, dims=1)/2)[1] +
              findmin(sum((ode_data[:, 1:50] .- trajectory_estimate_structured_O_NET[:, 1:50]).^2, dims=1)/2)[1])/2
...
begin
  min_error = (findmax(sum((test_data[:, 1:50] .- test_trajectory_estimate_structured_O_NET[:, 1:50]).^2, dims=1)/2)[1] +
              findmin(sum((test_data[:, 1:50] .- test_trajectory_estimate_structured_O_NET[:, 1:50]).^2, dims=1)/2)[1])/2
...
plt = plot!(tsteps, H_structured_O_NET, lw=3, label="Structured O-NET", ylims=(0.7, 0.83))
...
plt = plot!(test_tsteps, test_H_structured_O_NET, lw=3, label="O-NET", ylabel="Hamiltonian", xlabel="Time step")
