...
## set up parameters, initial conditions, ODEProblem, solve the ODEProblem, get the solution
p = [0.04,3e7,1e4]
u0 = [1.0,0.0,0.0]
...
function predict_adjoint(p)
    p = exp.(p)
    _prob = remake(prob,p=p) ## remake means use new parameters to replace the old parameters
