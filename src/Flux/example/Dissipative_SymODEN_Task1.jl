...
function TrueODEfunc_Task1(df,x,u,t) ### in-place usage
  ...
  p = x[2]
  dq = 3p
  dp = -5sin(q)-0.3p+u ### -0.3p is the dissipative part
  df[1] = dq
  df[2] = dp
end

## define an ODEProblem with the self-defined ODEFunction above 
prob_trueode_task1 = ODEProblem(TrueODEfunc_Task1, x0, tspan, 0)

## solve the ODEProblem
sol = solve(prob_trueode_task1, Tsit5(), saveat = tsteps)
...
dudt2 = FastChain(FastDense(2, 50, tanh), ### Multilayer perceptron for the part we don't know
...
DiffEqFlux.sciml_train(loss_neuralode, prob_neuralode.p, cb = callback)    
