...
begin
    using Lux, NNlib
    Structured_ODE_NN_d = Lux.Chain(Lux.Dense(2, 20, tanh),
                                    Lux.Dense(20, 10),
                                    Lux.Dense(10, 1))
    using Random
...
begin
    using Lux, NNlib
    Structured_ODE_NN_tc = Lux.Chain(Lux.Dense(2, 20, tanh),
                                     Lux.Dense(20, 10),
                                     Lux.Dense(10, 1))
    using Random
    rng = Random.default_rng()
...
begin
    ...
    function NeuralODE_Structured_ODE_NN(dz, z, θ_NN, t)
        ...
        dz[1] = v
        dz[2] = - q/c - Structured_ODE_NN_d([v, θ_d], θ_NN_d, st_Structured_ODE_NN_d)[1][1]*v
        dz[3] = Structured_ODE_NN_tc([θ_d, θ_0], θ_NN_tc, st_Structured_ODE_NN_tc)[1][1]*(Δθ)/θ_0
        dz[4] = Structured_ODE_NN_d([v, θ_d], θ_NN_d, st_Structured_ODE_NN_d)[1][1]*((v)^2)/θ_d - Structured_ODE_NN_tc([θ_d, θ_0], θ_NN_tc, st_Structured_ODE_NN_tc)[1][1]*(Δθ)/θ_d
    end
...
begin
    ...
    α_learn = 0.005
    epochs = 500
    θ_Structured_ODE_NN = LuxTrain(optf_Structured_ODE_NN, θ_Structured_ODE_NN, α_learn, epochs, dataloader1, callback_Structured_ODE_NN)
end
...
begin
    ...
    α_learn = 0.005
    epochs = 500
    θ_Structured_ODE_NN = LuxTrain(optf_Structured_ODE_NN, θ_Structured_ODE_NN, α_learn, epochs, dataloader2, callback_Structured_ODE_NN)
end
...
begin
    ...
    α_learn = 0.001
    epochs = 200
    θ_Structured_ODE_NN = LuxTrain(optf_Structured_ODE_NN, θ_Structured_ODE_NN, α_learn, epochs, dataloader3, callback_Structured_ODE_NN)
end
...
begin
    using JLD2, Lux
    path = joinpath(@__DIR__, "parameters", "params_compositional_experiment_ndho_with_EPHS_structure.jld2")
    JLD2.save(path, "params", θ_Structured_ODE_NN)
    path = joinpath(@__DIR__, "models", "compositional_experiment_ndho_with_EPHS_structure.jld2")
...
begin
    ...
    #Plots.pdf(joinpath(@__DIR__, "figures", "phase_portrait_compositional_ndho_with_EPHS_structure.pdf"))
end
...
begin
    ...
    plot(xlabel="Time Step", ylabel="L2 Error", xlims=(0,100), ylims=(0,0.000001))
    plot!(time_step, l2_error_Structured_ODE_NN, lw=3, label="Structured ODE NN")
    #Plots.pdf(joinpath(@__DIR__, "figures", "prediction_error_compositional_ndho_with_EPHS_structure.pdf"))
end
...
begin
    ...
    #Plots.pdf(joinpath(@__DIR__, "figures", "Hamiltonian_evolution_compositional_ndho_with_EPHS_structure.pdf"))
end
...
begin
    ...
    for idx in range(1,length(ode_data[2,:]))
        ...
        Δθ = θ_d - θ_0
        v = p/m
        f_θ_d__p__f[idx] = Structured_ODE_NN_d([v, θ_d], θ_NN_d, st_Structured_ODE_NN_d)[1][1]*v
        f_θ_d__s_d__f[idx] = - Structured_ODE_NN_d([v, θ_d], θ_NN_d, st_Structured_ODE_NN_d)[1][1]*((v)^2)/θ_d
        f_θ_tc__s_d__f[idx] = Structured_ODE_NN_tc([θ_d, θ_0], θ_NN_tc, st_Structured_ODE_NN_tc)[1][1]*(Δθ)/θ_d
        f_θ_tc__s_e__f[idx] = - Structured_ODE_NN_tc([θ_d, θ_0], θ_NN_tc, st_Structured_ODE_NN_tc)[1][1]*(Δθ)/θ_0
        estimate_dissipative_power_d[idx] = θ_0 * Structured_ODE_NN_d([v, θ_d], θ_NN_d, st_Structured_ODE_NN_d)[1][1] * v .^2 ./ θ_d
        estimate_dissipative_power_tc[idx] = θ_0 * Structured_ODE_NN_tc([θ_d, θ_0], θ_NN_tc, st_Structured_ODE_NN_tc)[1][1] * ((θ_d .- θ_0) ./ θ_0 .+ (θ_0 .- θ_d) ./ θ_d)    
    end
end
...
begin
    ...
    #Plots.pdf(joinpath(@__DIR__, "figures", "p.f_compositional_ndho_with_EPHS_structure.pdf"))
end

begin
    using Plots
    plot(xlabel="Time Step", ylabel="s_d.f of the damping", xlims=(0,100), ylims=(-0.001,0.0005))
    ...
    #Plots.pdf(joinpath(@__DIR__, "figures", "sd_d.f_compositional_ndho_with_EPHS_structure.pdf"))
end

begin
    using Plots
    plot(xlabel="Time Step", ylabel="s_d.f of the thermal conduction", xlims=(0,100), ylims=(-0.02,0.05))
    ...
    #Plots.pdf(joinpath(@__DIR__, "figures", "sd_tc.f_compositional_ndho_with_EPHS_structure.pdf"))
end

begin
    using Plots
    plot(xlabel="Time Step", ylabel="sₑ.f", xlims=(0,100), ylims=(-0.03,0.03))
    ...
    #Plots.pdf(joinpath(@__DIR__, "figures", "se.f_compositional_ndho_with_EPHS_structure.pdf"))
end
...
begin
    ...
    #Plots.pdf(joinpath(@__DIR__, "figures", "dissipative_power_damping_ndho_with_EPHS_structure.pdf"))
end

begin
    using Plots
    plot(xlabel="Time Step", ylabel="Dissipative Power of the thermal conduction", xlims=(0,100), ylims=(-0.5,1.0))
    ...
    #Plots.pdf(joinpath(@__DIR__, "figures", "dissipative_power_thermal_conduction_ndho_with_EPHS_structure.pdf"))
end


###########################################
# Reuse the models for a different system #
###########################################
...
begin
    using Main.DataHelper: ODEfunc_ndho, ODESolver
    # mass m, spring compliance c, damping coefficient d, environment temperature θ_0, heat transfer coefficient α, heat capacity c_tc
    params = [4, 1, 0.5, 300, 0.2, 1.0]
    # initial state q, p, s_e, s_d
    initial_state = [1.0, 2.0, 0.2, 5.8]
    # Generate data set
    time_span = (0.0, 99.9)
    time_step = range(0.0, 99.9, 1000)
    ode_data = ODESolver(ODEfunc_ndho, params, initial_state, time_span, time_step)
...
begin
    using Plots
    plot(xlabel="q", ylabel="p", xlims=(-3,4), ylims=(-3,4))
    plot!(ode_data[1,:], ode_data[2,:], lw=3, label="Ground Truth", linestyle=:solid)
    plot!(predict_data_Structured_ODE_NN[1,:], predict_data_Structured_ODE_NN[2,:], lw=3, label="Structured ODE NN", linestyle=:dash)
    #Plots.pdf(joinpath(@__DIR__, "figures", "phase_portrait_compositional_ndho_with_EPHS_structure_reuse.pdf"))
end
...
begin
    ...
    plot(xlabel="Time Step", ylabel="L2 Error", xlims=(0,100), ylims=(0,0.000001))
    plot!(time_step, l2_error_Structured_ODE_NN, lw=3, label="Structured ODE NN")
    #Plots.pdf(joinpath(@__DIR__, "figures", "prediction_error_compositional_ndho_with_EPHS_structure_reuse.pdf"))
end
...
begin
    ...
    plot!(time_step, round.(Hamiltonian_Ground_Truth, digits=10), lw=3, label="Ground Truth", linestyle=:solid)
    plot!(time_step, round.(Hamiltonian_Structured_ODE_NN, digits=10), lw=3, label="Structured ODE NN", linestyle=:dash)
    #Plots.pdf(joinpath(@__DIR__, "figures", "Hamiltonian_evolution_compositional_ndho_with_EPHS_structure_reuse.pdf"))
end
...
begin
    ...
    plot!(time_step, R_d__p__f, lw=3, label="Ground Truth", linestyle=:solid)
    plot!(time_step, f_θ_d__p__f, lw=3, label="Structured ODE NN", linestyle=:dash)
    #Plots.pdf(joinpath(@__DIR__, "figures", "p.f_compositional_ndho_with_EPHS_structure_reuse.pdf"))
end
...
begin
    ...
    plot!(time_step, R_d__s_d__f, lw=3, label="Ground Truth", linestyle=:solid)
    plot!(time_step, f_θ_d__s_d__f, lw=3, label="Structured ODE NN", linestyle=:dash)
    #Plots.pdf(joinpath(@__DIR__, "figures", "sd_d.f_compositional_ndho_with_EPHS_structure_reuse.pdf"))
end
...
begin
    ...
    plot!(time_step, R_tc__s_d__f, lw=3, label="Ground Truth", linestyle=:solid)
    plot!(time_step, f_θ_tc__s_d__f, lw=3, label="Structured ODE NN", linestyle=:dash)
    #Plots.pdf(joinpath(@__DIR__, "figures", "sd_tc.f_compositional_ndho_with_EPHS_structure_reuse.pdf"))
end
...
begin
    ...
    plot!(time_step, R_tc__s_e__f, lw=3, label="Ground Truth", linestyle=:solid)
    plot!(time_step, f_θ_tc__s_e__f, lw=3, label="Structured ODE NN", linestyle=:dash)
    #Plots.pdf(joinpath(@__DIR__, "figures", "se.f_compositional_ndho_with_EPHS_structure_reuse.pdf"))
end
...
begin
    ...
    plot!(time_step, target_dissipative_power_d, lw=3, label="Ground Truth", linestyle=:solid)
    plot!(time_step, estimate_dissipative_power_d, lw=3, label="Structured ODE NN", linestyle=:dash)
    #Plots.pdf(joinpath(@__DIR__, "figures", "dissipative_power_damping_ndho_with_EPHS_structure_reuse.pdf"))
end
...
begin
    ...
    plot!(time_step, target_dissipative_power_tc, lw=3, label="Ground Truth", linestyle=:solid)
    plot!(time_step, estimate_dissipative_power_tc, lw=3, label="Structured ODE NN", linestyle=:dash)
    #Plots.pdf(joinpath(@__DIR__, "figures", "dissipative_power_thermal_conduction_ndho_with_EPHS_structure_reuse.pdf"))
end